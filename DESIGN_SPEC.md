# JunkHaul — Complete UI Specification for Designer
...
| Button tap | — | spring (stiffness:260, damping:20) | `scale(0.97)` on press |
...
**Step 0 — Welcome / Install App**
- Invite validation (shows: "You're invited to join" + name + email + pay rate)
- "Add to Home Screen" button (icon: Share) — PWA install
- "Continue" button
...
- 4 upload cards, each with icon + label + status:
  ...
  - Selfie (icon: Camera) — camera viewfinder
- Each card: tap to upload → shows thumbnail preview → status pill (Uploaded/Verified/Rejected)
- Rejection reason shown if rejected
- "Continue" button (enabled when all uploaded)

**Step 3 — TD1 Federal Tax Form**
...
**Step 6 — Banking Info (for direct deposit)**
...
**Purpose**: Main hub — Uber-driver-style map + job cards
...
- Right: Icon buttons — Clock (→ /portal/clock), FileText (→ /portal/documents), Wallet (→ /portal/paystubs), Bell with unread badge (→ /portal/notifications), LogOut
...
- **Today view** (default):
  ...
  - Earnings estimator: hours × pay_rate = estimated pay
- **Weekly view** (toggle):
...
- **Bottom toolbar**:
  ...
  - Report Incident (icon: AlertTriangle, → /portal/incidents)
...
**Header**: Back button (icon: ArrowLeft → /portal/schedule), "Report Issue" button (icon: Flag)

**Step 1 — En Route**
- Job address (icon: MapPin)
- Customer name
- Items list
- "Mark En Route" button (icon: Navigation) — triggers GPS tracking
- GPS coordinates display

**Step 2 — Arrived**
- "Mark Arrived" button (icon: MapPin) — captures GPS, auto clock-in
...
- Take item photos (icon: Camera) — camera viewfinder, multiple photos
- Photo thumbnails preview

**Step 5 — Route to Drop**
...
- Take capacity photo (icon: Camera) — photo of truck after dump
- Photo preview
...
**Step 8 — End of Day (EOD)**
- Odometer reading input
- Fuel level input
- Take dash photo (icon: Camera) — dashboard odometer
- Take gas receipt photo (icon: Camera)
- Take dump receipt photo (icon: Camera)
- Gas amount input ($)
- Dump amount input ($)
- "Submit EOD" button (icon: Send) — auto clock-out
...
- Header buttons: Today (→ /portal/schedule), Docs (→ /portal/documents), Pay (→ /portal/paystubs), LogOut
...
**Header**: Back (icon: ArrowLeft → /portal/schedule), LogOut
...
- Expiry badge (if applicable): color-coded — green (>30 days), amber (<30 days), red (<7 days), "Expired" (red)
- Rejection reason (if rejected, expandable)
- Upload button (icon: Upload) — file picker (image or PDF)
...
**Header**: Today (icon: Calendar → /portal/schedule), LogOut
...
**Header**: Back (icon: ArrowLeft → /portal/schedule), "Mark all read" button (icon: CheckCheck, only when unread > 0)
...
- Tap → marks read + navigates to link
...
**Header**: Back (icon: ArrowLeft → /portal/schedule), "+ Report" button (icon: AlertTriangle)
...
2. **AI Agent Chat**:
   ...
   - Action results with ✓/✗ indicators
3. **AI Briefing**:
   - ✦ header + "cached" badge + age ("5 min ago")
   - "Regenerate" button
...
**Top bar**: Date selector (horizontal scroll of dates), "🗺️ Optimise route" button
...
1. **Abandonment funnel** (visual bars):
   - Quoted → T+1hr touch → T+20hr touch → T+47hr touch → Booked (converted)
...
**Employee list**:
- Each row: Name, Status badge (active=green, pending=amber, terminated=red), Clocked-in indicator (pulsing green dot + duration), Email, Onboarding badges (Contract ✓, TD1 ✓, Ack ✓), Pay rate, Period minutes
- Tap → opens slide-over panel
...
- Recent clock sessions: booking ID, clock in → clock out, duration
...
- Vertical timeline of events with emoji icons:
  - ⚡ surge_applied, 📍 offer_sent, 💬 abandonment_touch, ⭐ review_request, ⬇️ sms_inbound, ↙ sms_outbound, 💰 offer, ⚙️ system_event
...
## Progress Indicators
- **Dots**: 6px wide × 3px tall, orange (completed) / `rgba(255,255,255,0.06)` (pending)
...
- Step tracker (Booked → Confirmed → En Route → Arrived → In Progress → Completed)
...
## Crew APIs
- `GET /api/employee/schedule` — today's assignments + weekly view
- `POST /api/crew/complete-job` — mark job complete with photos/signature/EOD
- `POST /api/crew/collect-payment` — record payment
- `GET /api/employee/notifications` — notification list
- `POST /api/employee/notifications` — mark read
- `POST /api/employee/issues` — submit issue flag
- `POST /api/employee/incidents` — submit incident report
- `GET /api/employee/onboard/invite` — validate invite token
- `POST /api/employee/onboard/invite` — accept invite + create account

## Admin APIs
- `GET/POST /api/admin/crew` — list employees / send invite
- `GET/PATCH/DELETE /api/admin/crew/[id]` — employee detail / update / terminate
- `POST /api/admin/crew/[id]/resend-invite` — resend onboarding invite
- `POST /api/admin/crew/assignments` — create assignment
- `GET/POST /api/admin/crew/storage` — storage facilities
- `GET/POST /api/admin/crew/donation-centers` — donation centers
- `GET/POST /api/admin/config` — system configuration
- `GET /api/admin/events` — audit trail
- `GET /api/admin/call-history` — call logs
- `GET /api/admin/referrals` — referral tracking
- `GET /api/admin/insights` — quadrant profit
- `GET /api/admin/growth` — growth funnel
- `GET /api/admin/command-center` — AI briefing + stats
- `POST /api/admin/agent` — AI agent chat
- `GET /api/admin/payroll/preview` — payroll preview
- `POST /api/admin/payroll/approve` — approve payroll
- `POST /api/admin/payroll/run` — run payroll
- `GET /api/admin/t4s` — T4 tax forms
- `GET /api/admin/remittance` — tax remittance
- `GET /api/admin/earnings` — earnings overview
- `GET /api/admin/bookings` — booking list
- `GET /api/admin/bookings/[id]/timeline` — booking event timeline
- `POST /api/admin/cancel` — cancel booking
- `POST /api/admin/complete` — mark booking complete
- `POST /api/admin/no-show` — mark no-show
- `POST /api/admin/mark-arrived` — mark crew arrived
- `GET /api/admin/leads` — lead list
- `GET /api/admin/quadrant-profit` — quadrant analysis
