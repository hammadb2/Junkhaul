...
async function createSmsBooking({ name, phone, address, load_size, job_date, job_time, stairs, has_freon, freon_count }) {
  ...
  // Operator alert — SMS bookings never sent this (audit C1); only the web
  // flow (via the Stripe deposit webhook's handleBookingConfirmed) did.
  try {
    await sendOperatorAlert(booking);
  } catch (e) {
    console.error('[sms-webhook] operator alert failed:', e.message);
  }
    ...
    if (photoBase64s.length > 0) {
      ...
      try {
        ...
        if (analysis.photo_unusable) {
          await sendSMS(from, 'Sorry, that photo didnt come through usable. Could you retake it and send it again?', recentBooking?.id, 'photo_unusable');
          return NextResponse.json({ ok: true });
        }

        // Safety alert: route privately to operator, then strip from anything
        // customer-facing (including the stored analysis below).
        await handleSafetyAlert(analysis, { source: 'sms', booking_id: recentBooking?.id || null, lead_phone: from });
        ...
        // Store analysis for booking flow (internal fields already stripped)
        await supabaseAdmin.from('messages').insert({
          booking_id: recentBooking?.id || null,
          direction: 'outbound',
          to_number: from,
