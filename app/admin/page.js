...
// ============================================================
// CREW PORTAL VIEW — employee onboarding, clock-in status,
// period hours, payroll run, CRA remittance, docs, T4s.
// ============================================================
function CrewPortalView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [empRes, remRes, runRes, t4Res] = await Promise.all([
      fetch('/api/admin/employees'),
      fetch('/api/admin/remittance'),
      fetch('/api/admin/payroll/approve'),
      fetch('/api/admin/t4s'),
    ]);
    const empData = await empRes.json();
    const remData = await remRes.json();
    const runData = await runRes.json();
    const t4Data = await t4Res.json().catch(() => ({ t4_slips: [] }));
    setEmployees(empData.employees || []);
    setSummary(empData.summary || null);
    setRemittance(remData);
    setPayRuns(runData.pay_runs || []);
    setT4s(t4Data.t4_slips || []);
    if (!period.start) {
      const d = new Date();
  ...
  const viewDocs = async (emp) => {
    setSelectedEmp(emp);
    setEmpDocs(null);
    const res = await fetch(`/api/admin/employee-docs?employee_id=${emp.id}`);
    const data = await res.json();
    setEmpDocs(data.documents || []);
  };

  const verifyDoc = async (docId, status) => {
    await fetch('/api/admin/employee-docs', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ document_id: docId, status }),
    });
    if (selectedEmp) viewDocs(selectedEmp);
    load();
  };

  const previewPayroll = async () => {
    setRunning(true); setPayrollMsg(''); setPreview(null);
    ...
    const data = await res.json();
    setRunning(false);
    if (data.pay_run) setPreview(data.pay_run);
    else setPayrollMsg(data.message || 'No shifts to pay');
  };

  const runPayroll = async () => {
    if (!confirm('Run payroll for this period? This calculates and saves pay stubs and tags shifts as paid.')) return;
    setRunning(true); setPayrollMsg('');
    ...
    setRunning(false);
    if (res.ok) {
      setPayrollMsg(`Pay run created: ${data.pay_run?.stubs?.length || 0} stubs. CRA remittance: $${Number(data.pay_run?.totals?.total_cra_remittance).toFixed(2)}`);
      setPreview(null);
      load();
    } else setPayrollMsg(data.error || 'Payroll run failed');
  };

  const approveRun = async (runId, sendDD) => {
    if (!confirm(sendDD ? 'Approve AND send direct deposits?' : 'Approve this pay run?')) return;
    const res = await fetch('/api/admin/payroll/approve', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ pay_run_id: runId, send_direct_deposit: sendDD }),
    });
    const data = await res.json();
    if (res.ok) {
      alert(data.deposits ? `Approved. Deposits: ${data.deposits.filter((d) => d.ok).length} sent, ${data.deposits.filter((d) => !d.ok).length} failed` : 'Approved');
      load();
    } else alert(data.error || 'Approve failed');
  };

  const markRemittancePaid = async (id) => {
  ...
  return (
    <div className="space-y-4">
      {summary && (
        <div className="grid grid-cols-4 gap-2">
          <Stat label="Crew" value={summary.total} />
          <Stat label="Onboarded" value={summary.onboarded} />
      ...
      {remittance && remittance.owed_count > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm">
          <div className="font-semibold text-amber-800">CRA remittance owed: ${remittance.owed_total.toFixed(2)}</div>
          <div className="text-amber-700 text-xs">
            {remittance.owed_count} pay run(s) · due {remittance.next_due ? new Date(remittance.next_due.due_date).toLocaleDateString() : 'soon'}
            {' · '}<button onClick={() => markRemittancePaid(remittance.next_due.id)} className="underline">Mark paid</button>
          </div>
        </div>
      )}

      <div className="bg-white rounded-xl border border-gray-200 p-3">
        <div className="font-semibold text-gray-900 text-sm mb-2">Run payroll</div>
        <div className="flex gap-2 items-center text-sm">
          <input type="date" value={period.start} onChange={(e) => setPeriod({ ...period, start: e.target.value })} className="px-2 py-1.5 rounded-lg border border-gray-200 text-xs" />
          <span className="text-gray-400">to</span>
          <input type="date" value={period.end} onChange={(e) => setPeriod({ ...period, end: e.target.value })} className="px-2 py-1.5 rounded-lg border border-gray-200 text-xs" />
          <button onClick={previewPayroll} disabled={running} className="ml-auto bg-gray-100 text-gray-700 text-xs font-semibold px-3 py-1.5 rounded-lg">{running ? '…' : 'Preview'}</button>
        </div>
        {payrollMsg && <div className="text-xs text-gray-500 mt-2">{payrollMsg}</div>}
        {preview && (
          <div className="mt-3 border-t border-gray-100 pt-3">
            <div className="grid grid-cols-4 gap-2 text-center text-xs mb-2">
              <div><div className="font-bold text-gray-900">{preview.stubs.length}</div><div className="text-gray-400">stubs</div></div>
              <div><div className="font-bold text-gray-900">${preview.totals.total_gross.toFixed(0)}</div><div className="text-gray-400">gross</div></div>
              <div><div className="font-bold text-gray-900">${preview.totals.total_net.toFixed(0)}</div><div className="text-gray-400">net</div></div>
              <div><div className="font-bold text-gray-900">${preview.totals.total_cra_remittance.toFixed(0)}</div><div className="text-gray-400">to CRA</div></div>
            </div>
            <button onClick={runPayroll} disabled={running} className="w-full bg-orange-500 text-white text-sm font-semibold py-2 rounded-lg disabled:bg-orange-300">
      ...
      {payRuns.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-3">
          <div className="font-semibold text-gray-900 text-sm mb-2">Pay runs</div>
          <div className="space-y-2">
            {payRuns.slice(0, 5).map((r) => (
              <div key={r.id} className="flex items-center justify-between text-sm">
                <div>
                  <div className="text-gray-900">{r.period_start} → {r.period_end}</div>
                  <div className="text-xs text-gray-400">${Number(r.total_net).toFixed(2)} net · CRA ${Number(r.total_cra_remittance).toFixed(2)}</div>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${r.status === 'paid' ? 'bg-green-100 text-green-700' : r.status === 'approved' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>{r.status}</span>
                  {r.status === 'calculated' && (
                    <>
                      <button onClick={() => approveRun(r.id, false)} className="text-xs bg-gray-900 text-white px-2 py-1 rounded-lg">Approve</button>
                      <button onClick={() => approveRun(r.id, true)} className="text-xs bg-orange-500 text-white px-2 py-1 rounded-lg">Approve + Pay</button>
                    </>
                  )}
                </div>
              </div>
      ...
      {t4s.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-3">
          <div className="font-semibold text-gray-900 text-sm mb-2">T4 slips ({t4s[0]?.tax_year})</div>
          <div className="space-y-2">
            {t4s.map((t) => (
              <div key={t.id} className="flex items-center justify-between text-sm border-b border-gray-100 pb-2">
                <div>
                  <div className="text-gray-900">{t.employees?.name || 'Unknown'}</div>
                  <div className="text-xs text-gray-400">
                    Box 14: ${Number(t.employment_income).toFixed(2)} · CPP: ${Number(t.cpp_contribution).toFixed(2)} · EI: ${Number(t.ei_premium).toFixed(2)} · Tax: ${Number(t.income_tax_deducted).toFixed(2)}
                  </div>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full ${t.status === 'filed' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{t.status}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="font-semibold text-gray-900 text-sm p-3 border-b border-gray-100">Crew</div>
        {loading ? (
          <p className="text-gray-400 text-center py-6 text-sm">Loading…</p>
        ) : employees.length === 0 ? (
          <p className="text-gray-400 text-center py-6 text-sm">No employees yet. Share /portal to onboard.</p>
        ) : (
            ...
            {employees.map((e) => (
              <div key={e.id} className="px-3 py-2.5 flex items-center gap-3 text-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-400">
                    {e.clocked_in ? `in ${e.clock_in_duration_min}m` : `${e.period.total_hours.toFixed(1)}h this period`}
                    {e.period.overtime_hours > 0 && ` · ${e.period.overtime_hours.toFixed(1)} OT`}
                    {!e.onboarding.complete && ` · missing ${e.onboarding.missing.length} doc(s)`}
                  </div>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full ${e.status === 'active' || e.status === 'onboarded' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{e.status}</span>
                <button onClick={() => viewDocs(e)} className="text-xs text-gray-500 underline">docs</button>
              </div>
            ))}
          </div>
        )}
      ...
      {selectedEmp && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={() => setSelectedEmp(null)}>
          <div className="bg-white rounded-2xl max-w-md w-full p-4 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <div className="font-bold text-gray-900">{selectedEmp.name}</div>
              <button onClick={() => setSelectedEmp(null)} className="text-gray-400">✕</button>
            </div>
            {!empDocs ? (
              ...
              <div className="space-y-2">
                {empDocs.map((d) => (
                  <div key={d.id} className="flex items-center justify-between text-sm border-b border-gray-100 pb-2">
                    <div>
                      <div className="text-gray-900">{d.doc_type.replace(/_/g, ' ')}</div>
                      <div className="text-xs text-gray-400">{d.status}{d.drive_file_url && ' · in Drive'}</div>
                    </div>
                    {d.status === 'uploaded' && (
                      <div className="flex gap-1">
                        <button onClick={() => verifyDoc(d.id, 'verified')} className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">Verify</button>
                        <button onClick={() => verifyDoc(d.id, 'rejected')} className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded">Reject</button>
                      </div>
                    )}
                  </div>
                ))}
