# Every Piece of Logic in the App ÔÇö Junk Haul Calgary
...
3. Add stairs_fee = stairs_count ├ù $25
4. Add freon_fee = freon_count ├ù $40 (if has_freon, min 1 item)
...
- Margin = profit / total_price ├ù 100

### Weight Safety Check
- If estimated weight ÔëÑ 100% of limit: HARD flag, "call customer before confirming"
- If estimated weight ÔëÑ 85% of limit: SOFT flag, "note in booking"
- Limits: single=150kg, quarter=300kg, half=500kg, full=700kg

### Landfill Selection
- Sunday jobs ÔåÆ East Calgary Landfill (only one open Sundays)
- Saturday jobs ÔåÆ Spyhill Landfill
- Weekdays ÔåÆ East Calgary Landfill
...
- High risk ÔåÆ extra reminder SMS sent before pickup
...
- STOP/UNSUBSCRIBE ÔåÆ opt-out confirmation
- HELP ÔåÆ company info + opt-out instructions
...
1. **quote_given + customer says yes** ÔåÆ ask for address
2. **awaiting_address + looks like address** ÔåÆ store temp_address, ask for name
3. **awaiting_name + valid name** ÔåÆ look up temp_address, get available slots, show up to 4 slots, store temp_name + temp_slots
4. **awaiting_slot_choice + number 1-4** ÔåÆ look up temp name/address/slots/photo_analysis, create booking, send deposit link, delete temp messages
5. **deposit_sent + says paid** ÔåÆ confirm "we got it, you're all set"
6. **deposit_sent + says no** ÔåÆ "no worries, link doesn't expire"
...
### Nominatim (OpenStreetMap) ÔÇö free, no API key
...
- Checks last call ÔÇö if it was an angry complaint, auto-routes to Morgan (manager)
- Otherwise routes directly to the right department agent (no greeter transfer)

**Department numbers:**
- Sales (+14127149201) ÔåÆ Casey
- Service (+14127149625) ÔåÆ Jordan
- Refunds (+14127149181) ÔåÆ Riley
- Manager (+14127149656) ÔåÆ Morgan

**Routing logic:**
1. Has refund request ÔåÆ Riley (refunds)
2. Has service request ÔåÆ Jordan (service)
3. Has booking with status pending_payment or confirmed ÔåÆ Jordan (service)
4. New caller or anything else ÔåÆ Casey (sales)
...
1. **check_availability** ÔÇö queries schedule table for open slots, grouped by date
2. **get_quote** ÔÇö calculates price for a given load size + addons
3. **create_booking** ÔÇö creates a booking (checks slot, calculates price, geocodes, inserts, sends deposit link via SMS)
4. **lookup_booking** ÔÇö finds booking by ref or phone, returns details
5. **cancel_booking** ÔÇö calls cancelBooking() with full policy logic
6. **reschedule_booking** ÔÇö calls rescheduleBooking() with max-2 check
7. **add_to_waitlist** ÔÇö calls addToWaitlist()
8. **issue_refund** ÔÇö processes Stripe refund (full or partial), updates booking
9. **send_email** ÔÇö sends branded HTML email via Resend
10. **escalate_to_human** ÔÇö sends SMS to operator, logs to phone_calls
11. **notify_operator** ÔÇö sends urgent SMS to operator
12. **get_calgary_info** ÔÇö live weather (Open-Meteo), CBC Calgary news RSS, seasonal events, traffic info
13. **get_sports_info** ÔÇö live NHL standings (Flames), Flames game schedule, Stampeders CFL info, CBC sports RSS
14. **get_job_photos** ÔÇö looks up booking photos by phone or booking_id (for Morgan)
15. **get_booking_details** ÔÇö full booking details lookup (for Morgan)
16. **escalate_to_owner** ÔÇö logs escalation, sends SMS to operator (for Morgan)
17. **log_compensation** ÔÇö logs compensation type (free removal, partial refund, full refund, return pickup) to compensation_log table (for Morgan)
...
- POST: upload a doc (multipart form)
  ...
  - Check if all 5 required docs are uploaded ÔåÆ if yes, update employee status to 'onboarded'
...
- Overtime: 1.5├ù regular rate for hours > 8/day OR > 44/week (whichever gives greater OT)
- 3-hour minimum: if reported and worked < 3 hrs, pay max(workedPay, 3 ├ù $15)
...
2. regPay = regularHours ├ù rate
3. otPay = overtimeHours ├ù rate ├ù 1.5
4. gross = regPay + otPay
5. If reported and dailyHours > 0 and < 3: gross = max(gross, 3 ├ù $15)

### CPP Calculation (Chapter 6)
- C = min(cap, gross)
- cap = cpp_max_contribution ├ù (PM/12) - ytdCPP
- gross = cpp_rate ├ù (PI - cpp_basic_exemption / P)
- Rounded half-up to nearest cent

### CPP2 Calculation (second additional)
- C2 = min(cap, gross)
- cap = cpp2_max_contribution ├ù (PM/12) - ytdCPP2
- W = max(PIYTD, cpp2_lower_ceiling ├ù PM/12)
- gross = (PIYTD + PI - W) ├ù cpp2_rate
...
- F5 = CPP enhancement per period = C ├ù (first_additional_rate / cpp_rate) + C2
- A = P ├ù (I - F5) [annualized taxable income]
- BPAF = dynamic basic personal amount (phases out for high income: $181,440 to $258,482)
- Find bracket for A
- T3 = R ├ù A - K - K1 - K2 - K4
  - K1 = lowest_rate ├ù TC (federal TD1 claim)
  - K2 = lowest_rate ├ù (baseCPP_annualized + EI_annualized)
  - K4 = min(lowest_rate ├ù A, lowest_rate ├ù CEA)
- T1 = T3 (no LCF)

### Alberta Tax (Steps 4-5)
- T4 = V ├ù A - KP - K1P - K2P - K5P
  - K1P = ab_lowest_rate ├ù TCP (Alberta TD1 claim)
  - K2P = ab_lowest_rate ├ù (baseCPP_annualized + EI_annualized)
  - K5P = max(0, (K1P + K2P - 4896) ├ù 0.25) [Alberta tax reduction]
...
- Each adapter: sendPayment({ amount, employee, reference }) ÔåÆ { txnId, status, raw }
...
### Per-Employee Folder
- Creates folder: "{name} ÔÇö {email}" under root
...
### Formatting
- formatTime: '15:00' ÔåÆ '3:00 PM'
- formatDateLong: "Thursday, July 3"
- dayType: Sunday ÔåÆ 'sunday', everything else ÔåÆ 'thursday' (pickup days)
