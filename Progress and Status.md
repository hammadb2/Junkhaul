...
- Production status: `PARTIAL`; storage and submission foundation works. Real vision analysis and route-fit now exist — see "Donation Intelligence and Route-Fit Backend" below — but that work has not been merged or deployed.
- Known gaps: this section describes the PR #2 foundation only (rule-based pre-screen, table-only route-fit). Superseded by the Donation Intelligence and Route-Fit Backend phase below. Abandoned-draft cleanup is documented but not built.

### Donation Intelligence and Route-Fit Backend

- Status: `BUILT_NOT_VERIFIED`
- Repository path: `supabase/migrations/20260728000001_donation_intelligence_dispatch.sql`, `lib/donationVision.js`, `lib/donationPhotoSufficiency.js`, `lib/donationCapacity.js`, `lib/donationRouteFit.js`, `lib/donationDestinations.js`, `lib/donationRouteProposals.js`, `lib/donationManagerScope.js`, `lib/donationVapiTools.js`, `lib/ai.js` (extended), `app/api/donation-request/route.js` (rewired), `app/api/admin/donations/**`, `components/admin/DonationsView.js`, `app/api/vapi/route.js` (rewired)
- API: `/api/admin/donations/[id]` (consolidated detail), `/api/admin/donations/[id]/analysis`, `/api/admin/donations/[id]/items/[itemId]`, `/api/admin/donations/[id]/capacity`, `/api/admin/donations/[id]/destinations`, `/api/admin/donations/[id]/route-fit`, `/api/admin/donations/route-proposals`, `/api/admin/donations/route-proposals/[id]`
- Database table: extends `donation_ai_analyses`, `donation_request_items`, `donation_centers`, `donation_route_matches`, `donation_requests`; adds `donation_photo_sufficiency`, `donation_capacity_estimates`, `donation_destination_scores`, `donation_route_proposals`; grants the pre-seeded `donations.route_match` permission to the manager role.
- Admin visibility: `COMPLETE_AND_VERIFIED` in code (AI recommendation vs final human decision shown as separate blocks, items/capacity/destinations/route-fit/proposals/Quo history/timeline/audit all in one panel); not verified against a live rendered browser (no Supabase credentials in the build environment — see Known gaps).
- Manager visibility: `COMPLETE_AND_VERIFIED` in code; route-fit and route-proposal endpoints enforce `donations.route_match` plus an explicit `manager_scopes` check (crew_assignment/crew/date) via `lib/donationManagerScope.js`, exercised by the integration test's permission matrix (owner/admin/in-scope manager/out-of-scope manager/employee/unauthenticated).
- Quo integration: `COMPLETE_AND_VERIFIED`; reuses the existing central `sendSMS`/`createExpectedReply`/STOP-suppression stack for specific (never vague) missing-photo requests — no second SMS sender was created.
- Vapi integration: `PARTIAL`; backend tool contracts only (`lib/donationVapiTools.js`, wired into `app/api/vapi/route.js`'s customer-facing dispatch) — no assistant configuration or production cutover, per phase scope.
- App impact: `NO_APP_IMPACT`; this phase is web/admin backend only, no crew Flutter app changes.
- Tests: `BUILT_NOT_VERIFIED`; `tests/donation-intelligence.test.js` (pure unit tests — vision parsing/normalization, photo sufficiency, capacity heuristics, route-fit hard rules via an extracted pure `decideRouteFit()`, destination scoring, permission-set checks) passes under `npm test`. `tests/integration/donation-intelligence.integration.js` (permission matrix, route-proposal approval/versioning, stale-proposal rejection, capacity manual-override precedence, audited destination override) is written against the same disposable-Supabase-project contract as `tests/integration/foundation.integration.js` but reports `NOT_RUN` in this environment (no `TEST_SUPABASE_*` credentials available) — never executed against a live project.
- Manual verification: `BUILT_NOT_VERIFIED`; `npm ci`, `npm test`, `npm run migrations:check`, `npm run lint`, `npm run build` all pass on Node 22.23.1 in this environment (migration additionally validated by replaying the full 43-file migration chain against a scratch local Postgres 16 with stubbed Supabase auth/storage/net/cron schemas). No live Supabase project, so `test:integration`, the admin UI, and real Gemini/Groq vision calls were never exercised end-to-end.
- Production status: `NOT_BUILT`; branch `agent/donation-intelligence-dispatch` is not merged, no migration has been applied to any real Supabase project, and no Vapi assistant was configured or deployed.
- Known gaps: no live-browser verification of the admin UI (missing Supabase credentials); vision pipeline never called live Gemini/Groq APIs (would need `GEMINI_API_KEY`/`GROQ_API_KEY` plus real uploaded photos); route-fit's "crew shift length so far" and "existing route minutes" are a rough per-stop estimate (45min customer / 20min other), not pulled from real job-duration data; destination operating-hours parsing is a simple per-weekday lookup, not a full hours-with-exceptions model; Vapi tool contracts have no assistant wired up to actually call them yet.
