...
// Format the pickup time for customer-facing messages.
// If the booking has window info, show "between 7:30 AM-11:00 AM".
// Otherwise fall back to the exact time.
const formatJobTime = (booking) => {
  if (booking.job_window_start && booking.job_window_end) {
    return `between ${formatTime(booking.job_window_start)}-${formatTime(booking.job_window_end)}`;
  }
  return `at ${formatTime(booking.job_time)}`;
};
...
${dayName(booking.job_date)}, ${monthName(booking.job_date)} ${dayOfMonth(booking.job_date)} ${formatJobTime(booking)}
${booking.address}

Total: $${booking.total_price}
Deposit paid: $${booking.deposit_amount || 50}
...
// MSG 17 — deposit link (for phone bookings via Vapi or WhatsApp)
export const sendDepositLink = async (booking) => {
  const ref = (booking.booking_ref || '').replace(/-/g, ' ');
  const body = `Hi ${booking.name}! Your Junk Haul Calgary pickup is set for ${formatDateLong(booking.job_date)} ${formatJobTime(booking)}.

Pay your $50 deposit here to lock it in:
${SITE}/pay/${booking.id}

Ref: ${ref}
Your slot is held for 2 hours. If we dont get payment by then, we may release it.`;

  // Send via WhatsApp if booking came from WhatsApp, otherwise SMS
  if (booking.source === 'whatsapp') {
    try {
      const { sendWhatsApp } = await import('./whatsapp');
      await sendWhatsApp(booking.phone, body, booking.id, 'deposit_link');
    } catch {
...
**Time:** ${booking.job_window_start ? formatJobTime(booking) : formatTime(booking.job_time)}
