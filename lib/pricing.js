...
import { calculateRouteCost, fromCents } from './costLedger.js';
...
export async function estimateJobCostAsync({ load_size, quadrant, asOf = new Date().toISOString() }) {
  const distanceKm = QUADRANT_DISTANCE_KM[quadrant] || 25;
  const cost = await calculateRouteCost({
    routeType: 'junkhaul_dirty',
    bookings: [{ id: 'est', load_size, total_price: 0 }],
    distanceKm,
    asOf,
  });
  return {
...
export async function estimateProfitAsync({ load_size, total_price, quadrant, asOf }) {
  const costs = await estimateJobCostAsync({ load_size, quadrant, asOf });
  const profit = total_price - costs.total_cost;
  return {
    ...costs,
    revenue: total_price,
