...
export async function quoteCustomerPrice({
  ...
  // Weight actually bound for the landfill, excluding e-waste and other
  // diverted material (Pricing Engine Phase 8) — falls back to weight_kg
  // when not supplied (e.g. a text-description-only quote with no
  // per-item routing breakdown). weight_kg above still drives truck
  // sizing regardless, since diverted items still need to be hauled.
  landfill_weight_kg,
  pricingConfig,
  ...
  const booking = {
    ...
    landfill_weight_kg: landfill_weight_kg != null ? Number(landfill_weight_kg) : undefined,
  };
