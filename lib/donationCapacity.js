...
// ------------------------------------------------------------
// deriveCapacityEstimate — PURE derivation from donation_request_items
// rows + duration config, no I/O. Exported for direct unit testing of
// the stackability/fragility/crew/equipment/conservative-estimate
// heuristics without a database.
// ------------------------------------------------------------
export function deriveCapacityEstimate(items = [], durationConfig = {}) {
  const { basePickup = 20, baseLoading = 15, baseUnloading = 15 } = durationConfig;
  const rows = items || [];
...
// ------------------------------------------------------------
// computeCapacityEstimate — fetches items + config, derives an
// estimate via deriveCapacityEstimate(), and persists it as the next
...
export async function computeCapacityEstimate({ donationRequestId, actorId = null }) {
  ...
  const derived = deriveCapacityEstimate(items || [], { basePickup, baseLoading, baseUnloading });
