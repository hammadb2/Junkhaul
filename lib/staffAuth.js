...
export async function hasManagerScope(employeeId, scopeType, scopeValue) {
  ...
  const { data } = await supabaseAdmin
    .from('manager_scopes')
    .select('id,effect,priority,expires_at')
  ...
  const rows = data || [];
  if (rows.some((scope) => scope.effect === 'deny')) return false;
  return rows.some((scope) => scope.effect === 'allow');
}

export async function managerCanAccessAny(employeeId, scopes = []) {
  for (const scope of scopes) {
    if (await hasManagerScope(employeeId, scope.scope_type, scope.scope_value)) return true;
  }
  return false;
}
