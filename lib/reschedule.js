...
// ============================================================
// RESCHEDULE HANDLER (Section 12) — max reschedules per booking
// is tunable via system_config.
// ============================================================
...
export const rescheduleBooking = async (booking_id, new_date, new_time) => {
  ...
  const maxReschedules = await getNumberConfig('reschedule_max_count', 2);

  if (booking.reschedule_count >= maxReschedules) {
    return {
      success: false,
      error: `Maximum ${maxReschedules} reschedules per booking. Please cancel and rebook.`,
    };
  }
  ...
  await sendSMS(
    booking.phone,
    `Done! Your booking ${booking.booking_ref} is moved to ${formatDateLong(new_date)} at ${formatTime(new_time)}.\n\n${booking.address}\nBalance due on pickup: $${booking.balance_due}\n\njunkhaul.ca`,
    booking_id,
    'reschedule'
  );
