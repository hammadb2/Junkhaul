...
// Uses direct fetch to the Groq API (avoids SDK connection issues).
// ============================================================
...
If photo_unusable is true, return ONLY {"photo_unusable": true} with every other field null/empty/false and NO descriptive text.`,
    ...
    console.error('Groq analysePhotos error:', err.message);
      ...
      [
        {
          role: 'user',
          content: [
            // Show old photos first (labeled as "previous").
            ...oldPhotos_base64.map((photo) => ({
              type: 'image_url',
              image_url: { url: `data:image/jpeg;base64,${photo}` },
            })),
            // Then new photos (labeled as "current").
            ...
            {
              type: 'text',
              text: `You are comparing two sets of photos of junk for removal. The FIRST set is the customer's previous upload, and the SECOND set is their new upload.
...
If photo_unusable is true, return ONLY {"photo_unusable": true} with every other field null/empty/false and NO descriptive text.`,
    ...
    console.error('Groq analysePhotoDiff error:', err.message);
    throw new Error(`Photo diff analysis failed: ${err.message || 'unknown error'}`);
  }
};
