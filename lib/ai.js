...
// 'meta-llama/llama-4-scout-17b-16e-instruct' was deprecated/removed from
// Groq's lineup (confirmed live: 404 model_not_found on every call, which
// meant analyseDescription — the ONLY path for a customer typing their junk
// instead of uploading photos, no fallback — was completely broken).
// llama-3.3-70b-versatile is Groq's current general-purpose text model,
// used here for analyseDescription (text-only, no images).
//
// NOTE: analysePhotos/analysePhotoDiff also use this constant as their
// Gemini-outage fallback, but that call sends IMAGES — Groq's account here
// currently has no vision-capable model available at all (checked the live
// /v1/models list: no vision-preview/scout model exists anymore). That
// fallback path is effectively non-functional until Groq offers a vision
// model again or this is pointed at a different vision provider. Not fixed
// here since there's no valid model to fix it TO right now — flagging as a
// known gap rather than papering over it with a text-only model that would
// silently fail the same way on an image request.
...
// ------------------------------------------------------------
export async function geminiVisionCompletion(photos_base64, textPrompt, maxTokens = 800) {
...
// ------------------------------------------------------------
export async function groqChatCompletion(messages, maxTokens = 800) {
...
// ------------------------------------------------------------
// Groq chat completion WITH tool/function calling — Groq's API mirrors
// OpenAI's chat completions spec (including tools/tool_choice), so this
// is a thin pass-through, not a new protocol. Used by the conversational
// AI booking flow (app/api/chat-booking/route.js) to let the model
// decide when to check availability, get a quote, or create a booking,
// instead of us hand-parsing intent out of free text. Returns the raw
// assistant message (which may contain tool_calls) so the caller's own
// loop can execute tools and feed results back in.
// ------------------------------------------------------------
export async function groqToolCallCompletion(messages, tools, { maxTokens = 1000, temperature = 0.3 } = {}) {
  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) throw new Error('GROQ_API_KEY is not set');
  ...
  return { message: choice?.message || null, finish_reason: choice?.finish_reason, usage: data.usage || null };
}
...
WHOLE VEHICLES / VEHICLE SHELLS:
- Cars, trucks, vans, SUVs, motorcycles, mopeds, scooters, ATVs, snowmobiles, boats, jet skis, trailers, campers, RVs, golf carts, or any other intact vehicle
- We are a junk-removal crew with a 2-person crew and a cargo truck, not a licensed auto wrecker — landfills do not accept intact vehicles (fluids, tires, frame), and a whole vehicle physically cannot be carried into the truck. If asked about a vehicle, set hazmat_description to something like "We can't take whole vehicles — a licensed auto wrecker/scrap yard can help with that." Car PARTS (batteries, tires) are separately listed below and are also rejected.
- This is a categorical exclusion regardless of the vehicle's size, condition, or whether it "still runs" — it applies to the vehicle itself, not just wrecked/non-running ones.

ITEMS TOO HEAVY FOR A STANDARD 2-PERSON CREW:
- Any single item you estimate at over 100kg (220 lbs) that cannot be safely carried into the truck by 2 people without a dolly, ramp, or specialised equipment (e.g. a full-size safe, a grand piano, an engine block, a hot tub, poured concrete slabs, a gun safe). Flag these the same way as hazmat — is_hazmat=true, excluded from price, with hazmat_description explaining the weight and that it needs special arrangement.
- Ordinary large furniture (fridges ~70kg, sofas ~45kg, washers/dryers ~50kg) is fine and should NOT be flagged just for being bulky — this only applies to items beyond a safe 2-person lift.
...
Items over 100kg per unit are hard-rejected (see "ITEMS TOO HEAVY FOR A
STANDARD 2-PERSON CREW" below), not just flagged — a 2-person crew can't
lift those into the truck regardless of what tier the rest of the load is.
...
If photo_unusable is true, return ONLY {"photo_unusable": true} with every other field null/empty/false and NO descriptive text.`;
  ...
  try {
    const { text: content } = await geminiVisionCompletion(photos_base64, prompt, 800);
    return parseJson(content);
  } catch (geminiErr) {
    console.error('Gemini analysePhotos failed, falling back to Groq:', geminiErr.message);
    try {
      const { text: content } = await groqChatCompletion(
        [
          {
            role: 'user',
            content: [
...
If photo_unusable is true, return ONLY {"photo_unusable": true} with every other field null/empty/false and NO descriptive text.`;
  ...
  try {
    const { text: content } = await geminiVisionCompletion(allPhotos, prompt, 800);
    return parseJson(content);
  } catch (geminiErr) {
