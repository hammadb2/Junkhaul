...
// Weight actually bound for the landfill (Pricing Engine Phase 8) —
// excludes e-waste and other diverted material a real per-tonne landfill
// fee shouldn't be based on. Falls back to total weight when a booking
// doesn't carry a per-item routing breakdown (e.g. a flat load-size
// estimate with no itemized analysis) — same conservative "assume it's
// all landfill-bound" default as before this phase.
function landfillWeightKgForBooking(b) {
  return Number(b.landfill_weight_kg ?? weightKgForBooking(b));
}
...
export async function calculateRouteCost({
  ...
  // Uses LANDFILL-BOUND weight specifically (excludes e-waste and other
  // diverted material, Phase 8) — truck sizing above still uses total
  // weight, since diverted items still get physically hauled.
  const totalWeightKgForDisposal = bookings.reduce((s, b) => s + landfillWeightKgForBooking(b), 0);
