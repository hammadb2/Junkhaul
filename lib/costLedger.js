...
function computeStopServiceMinutes(stops) {
  for (const s of stops) {
    if (s.purpose === 'customer') {
      s.service_minutes = s.on_site_minutes ?? 30;
...
export async function calculateRouteCost({
  ...
  const cfg = costConfig || (await getCostConfig({ asOf, facility, wasteStream, client }));
  ...
  computeStopServiceMinutes(stops);
  ...
  // Fuel
  const fuelDollars = computeFuelCost({ fuelRate: cfg.fuel, distanceKm: totalKm });

  // Labor
  const laborDollars = computeLaborCost({ laborRate: cfg.labor, hours, people });

  // Disposal (only for dirty routes)
  const disposalDollars = routeType === 'junkhaul_dirty'
    ? computeFacilityCost({ facilityRate: cfg.facility, loadSize: bookings[0]?.load_size })
    : 0;
  ...
  const breakdown = {
    ...
    disposal: {
      ...
      predicted_weight_kg: bookings.reduce((s, b) => s + weightKgForBooking(b), 0),
