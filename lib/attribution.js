...
export async function captureAttribution({
  ...
  if (lead_id) await supabaseAdmin.from('leads').update(patch).eq('id', lead_id).catch(() => {});
  if (booking_id) await supabaseAdmin.from('bookings').update(patch).eq('id', booking_id).catch(() => {});
  if (donation_request_id) await supabaseAdmin.from('donation_requests').update(patch).eq('id', donation_request_id).catch(() => {});
