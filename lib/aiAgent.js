...
import { isPaidStatus } from './paymentStatus';
import { placeVapiOutboundCall } from './vapiOutbound';
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          name: { type: 'string' },
          phone: { type: 'string' },
          preferred_day_type: { type: 'string', enum: ['weekday', 'saturday', 'sunday', 'either'] },
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        type: 'object',
        properties: {
          day_type: { type: 'string', enum: ['weekday', 'saturday', 'sunday'], description: 'Filter by day type' },
...
async function toolCallCustomer({ phone, agent_type, context }) {
  try {
    // In-process (audit C6), not a self-referential HTTP call to this
    // app's own production URL.
    ...
    if (result.ok) {
      ...
      return { success: true, message: `Call initiated to ${phone} with ${agent_type} agent`, call_id: result.call_id };
    } else {
      await logAction('call_customer', { phone, agent_type, context }, false, result.error);
      return { error: `Call failed: ${result.error || 'Unknown error'}` };
    }
  } catch (e) {
...
async function toolGetTodaySummary() {
  ...
  // payment_status has no plain 'paid' value (audit G3) — isPaidStatus is
  // the source of truth for "money actually collected".
  const revenueCollected = jobList.filter(b => b.status === 'completed' && isPaidStatus(b.payment_status)).reduce((s, b) => s + b.total_price, 0);
...
async function gatherOperationalContext() {
  ...
  try {
    ...
    const collected = jobList.filter(b => b.status === 'completed' && isPaidStatus(b.payment_status)).reduce((s, b) => s + b.total_price, 0);
