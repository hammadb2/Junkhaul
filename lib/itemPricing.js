...
//      wrecker, not a junk-removal landfill run.
//   2. Any single item heavier than a 2-person crew can safely carry
//      into a cargo van/U-Haul without a dolly, ramp, or extra labour —
//      these need special arrangement, not a standard job.
// ============================================================
...
const VEHICLE_FALSE_POSITIVES = /\b(car seat|carseat|boat shoe|boat neck|trailer hitch)\b/i;
...
export function checkItemEligibility(name, estimatedWeightKg) {
  ...
  if (weight > MAX_TWO_PERSON_LIFT_KG) {
    return {
      rejected: true,
      reason: `${name || 'This item'} (~${Math.round(weight)}kg) is too heavy for our 2-person crew to safely carry into the truck without special equipment. Call us to arrange it separately.`,
    };
  }
...
export const buildItemizedQuote = (items_detected, { stairs = 0, same_day = false } = {}) => {
  if (!items_detected || items_detected.length === 0) {
    return { items: [], subtotal: 0, stairs_fee: 0, same_day_fee: 0, total: 0, is_minimum: false, minimum_applied: 0, estimated_dump_fee: 0, rejected_items: [] };
  }
  ...
  const estimated_dump_fee = dumpItems.length === 0 ? 0 : Math.max(25, Math.round((acceptedWeightKg / 1000) * 120));
