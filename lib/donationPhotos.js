...
export async function inspectDonationImage(buffer, declaredMime) {
  ...
  if (meta.format === 'svg') throw new Error('SVG uploads are not allowed');
...
export async function storeDonationPhoto({ donationRequestId, token, file, photoType, replacePhotoId = null }) {
  ...
  if (replacePhotoId) {
    const { data: oldPhoto } = await supabaseAdmin.from('donation_request_photos')
      .select('storage_path')
    ...
    if (oldPhoto?.storage_path) {
      await supabaseAdmin.storage.from(DONATION_PHOTO_BUCKET).remove([oldPhoto.storage_path]).catch(() => {});
    }
  }
  ...
  const { data: row, error } = await supabaseAdmin
    ...
    .insert({
      ...
      duplicate_result: duplicate?.id ? `duplicate_of:${duplicate.id}` : 'unique',
