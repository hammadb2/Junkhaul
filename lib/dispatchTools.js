...
export const dispatchTools = {
  ...
  async get_crew_schedule({ phone, email, name, date }) {
    ...
    if (phone) employee = await findEmployeeByPhone(phone);
      ...
      if (!matches || matches.length === 0) return 'No crew member found with that name. Ask for their phone number or email instead.';
    ...
    if (!employee) return 'I couldnt find a crew member with that info. Ask for their phone number or email address.';
