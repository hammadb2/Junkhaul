...
import { getEffectiveCapacityEstimate } from './donationCapacity';
...
const AVG_SPEED_KMH = 40; // matches lib/routeOptimizer.js / lib/dispatch.js fallback assumption
const DEFAULT_TRUCK_EQUIPMENT = new Set(['dolly', 'moving_blankets', 'furniture_straps']);
...
function findBestInsertion(stops, pickup) {
...
// ------------------------------------------------------------
// evaluateRouteFit — evaluates ONE candidate crew_assignment/route.
//
// expectedRouteVersion: the route_version the caller last saw. If the
// live route_plans version has moved on, we reject immediately as a
// stale route version rather than silently evaluating a route the
// caller no longer has an accurate picture of.
// ------------------------------------------------------------
export async function evaluateRouteFit({ donationRequestId, crewAssignmentId, expectedRouteVersion = null, destinationCandidate = null }) {
  ...
  const { data: donationRequest } = await supabaseAdmin
  ...
  const { data: crewAssignment } = await supabaseAdmin
  ...
  // Active manager lock: another pending proposal already targets this
  // exact route version — first-come, first-served to avoid two AI
  // proposals racing to insert into the same route snapshot.
  const { data: lockingProposals } = await supabaseAdmin
    .from('donation_route_proposals')
    .select('id, donation_request_id')
    .eq('crew_assignment_id', crewAssignmentId)
    .eq('source_route_version', routePlan.route_version)
    .eq('status', 'pending')
    .neq('donation_request_id', donationRequestId);
  if (lockingProposals?.length) {
    reasons.push('active_manager_lock');
    return { decision: 'fits_another_route', reasons, route_version: routePlan.route_version };
  }
