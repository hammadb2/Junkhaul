...
function evidenceComplete(quoteInput) {
  // A staged quote is evidence-complete when either photos were provided,
  // an AI load estimate exists with reasonable confidence, or the customer
  // supplied a description and we are not in a high-uncertainty category.
...
export function decide({ quoteInput, priceCents, minimumPriceCents, contributionCents, marginPercent }) {
  const reasons = [];
...
export async function createQuoteDecision({
  ...
  const { state, reasons } = decide({ quoteInput, priceCents, minimumPriceCents, contributionCents, marginPercent });
  ...
  await recordTimelineEvent({
    ...
    metadata: { ref, price_cents: priceCents, minimum_price_cents: minimumPriceCents },
  });
