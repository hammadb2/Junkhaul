...
import { calculatePrice, getPricingConfig, LOAD_LABELS, PRICING } from './pricing';
...
export const vapiTools = {
  ...
  async get_quote({ load_size, same_day = false, stairs = 0, has_freon = false, freon_count = 0, address }) {
    ...
    const p = calculatePrice({ load_size, same_day, stairs, has_freon, freon_count, pricingConfig });
    ...
    const decision = await createQuoteDecision({ quoteInput, priceCents: toCents(p.total) });
  ...
  async create_booking({ name, phone, address, load_size, job_date, job_time, job_window_label = null, job_window_start = null, job_window_end = null, same_day = false, stairs = 0, has_freon = false, freon_count = 0 }) {
    ...
    const pricingConfig = await getPricingConfig();
    const priced = calculatePrice({ load_size, same_day, stairs, has_freon, freon_count, job_date, job_time, pricingConfig });
