...
export const vapiTools = {
  ...
  // 2 — price quote
  async get_quote({ load_size, same_day = false, stairs = 0, has_freon = false }) {
    if (!PRICING.loads[load_size]) return 'I need a valid load size: single item, quarter, half, or full.';
    const p = calculatePrice({ load_size, same_day, stairs, has_freon });
    return `A ${LOAD_LABELS[load_size]} is $${p.total} total. That's a $50 deposit to book, and $${p.balance_due} due on pickup day.`;
  },

  // 3 — create a booking (deposit paid via link)
  async create_booking({ name, phone, address, load_size, job_date, job_time, same_day = false, stairs = 0, has_freon = false }) {
