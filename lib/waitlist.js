...
export const notifyWaitlist = async (freed_date, freed_time) => {
  ...
  const body = `🚛 Good news ${entry.name}! A spot just opened at Junk Haul Calgary.

${dayName(freed_date)} ${monthName(freed_date)} ${dayOfMonth(freed_date)} at ${formatTime(freed_time)} is now available.

Reply YES to grab it (30 min window, deposit required)
...
export const addToWaitlist = async ({ name, phone, preferred_date, preferred_day_type, load_size, address }) => {
  const { data, error } = await supabaseAdmin
    .from('waitlist')
    .insert({ name, phone, preferred_date, preferred_day_type, load_size, address })
    .select()
    .single();
  ...
  await sendSMS(
    phone,
    `You're on the Junk Haul Calgary waitlist! We'll text you the moment a ${preferred_day_type === 'either' ? 'Thursday or Sunday' : preferred_day_type} spot opens up.`,
    null,
    'waitlist_confirm'
  );
