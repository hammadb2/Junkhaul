...
export const BLOCKED_STATUSES = new Set(['terminated', 'rejected', 'deletion_requested']);
...
export async function getSessionEmployee(token) {
  ...
  const { data: emp } = await supabaseAdmin
    ...
    .maybeSingle();
  if (!emp) return null;
  // Reject sessions for employees whose status has been changed to a
  // blocked state after login. Destroy the session so it can't be reused.
  if (BLOCKED_STATUSES.has(emp.status)) {
    await supabaseAdmin.from('employee_sessions').delete().eq('token', token);
    return null;
  }
  return emp;
...
export function sessionCookieHeader(token, expiresAt) {
  return `${SESSION_COOKIE}=${token}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=${SESSION_TTL_SECONDS}; Expires=${new Date(expiresAt).toUTCString()}`;
}
...
// ------------------------------------------------------------
// Booking assignment check.
// Returns true if the employee is the driver or secondary on a
// crew_assignment for the booking's job_date.
// ------------------------------------------------------------
export async function isEmployeeAssignedToBooking(employeeId, bookingId) {
  if (!employeeId || !bookingId) return false;
  ...
  if (!booking || !booking.job_date) return false;

  // Check if the employee has a crew_assignment for that date.
  const { data: assignment } = await supabaseAdmin
    .from('crew_assignments')
    .select('id')
    .eq('assignment_date', booking.job_date)
    .or(`driver_employee_id.eq.${employeeId},secondary_employee_id.eq.${employeeId}`)
    .maybeSingle();
