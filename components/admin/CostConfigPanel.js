...
const DEFAULT_RECORD = {
  rental_rate_versions: { provider: 'U-Haul', location: 'Gas Plus Balzac, 10070 Hwy 566, Balzac, AB T4B 2T3', daily_rate: 40, included_km: 0, per_mile_rate: 2.4, taxes_percent: 0 },
  fuel_rate_versions: { price_per_litre: 1.75, quote_safety_l_per_100km: 45, source: '' },
  labor_rate_versions: { role_or_employee: 'default_crew', hourly_rate: 18, burden_percent: 0, overtime_rules: {} },
...
export default function CostConfigPanel({ flash }) {
  ...
  async function saveVersion() {
    ...
    try {
      ...
      const expectedReplacedId = currentVersionForTable()?.id || null;
      const res = await fetch('/api/admin/cost-config', {
        method: 'POST',
  ...
  function buildPayload(includeAudit) {
    const r = { ...record, effective_from: effectiveFrom };
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 24 }}>
        <div>
          <h3 style={{ fontSize: 16, marginBottom: 12 }}>Create new version</h3>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: 'block', fontSize: 12, color: '#666', marginBottom: 4 }}>Versioned table</label>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
            ...
            <button onClick={saveVersion} disabled={saving} style={buttonStyle('#10B981')}>{saving ? 'Saving…' : 'Create version'}</button>
          </div>
